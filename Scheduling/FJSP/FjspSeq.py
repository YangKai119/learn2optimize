...
class JobNode():            # 每道工序的点
    def __init__(self, job_id, process_id, machines_ava, work_time, embedding=None):
        self.job_id = job_id
        self.machine_id = None
        self.machines_ava = machines_ava
...
class JobScheduleManager(JobManager):
    def __init__(self, num_machine, num_jobs ,num_process):
        ...
        self.machine_list = []
        self.sol_seq = [[] for _ in range(2)]    # 解的序列，双链结构，上层是MS序列，下层是OS序列
        self.cur_obj = 0
        self.fitness = []
        # self.best_obj = 0
    ...
    def get_proc_time_state(self, cur_proc ,job_id, proc_id, machine):   # 判断该工序是否可以完成，完成的条件为，之前的工序已经完成，且其完成时间小于当前的开始工作的开始时间
        if not proc_id or not machine.process_seq:    # 在机器的开头还要在工序的开头才有这个待遇
            ...
            cur_proc.finish_time = cur_proc.start_time + cur_proc.work_time[cur_proc.machine_id]
            return
    ...
    def machine_reset(self, machine_id = None):     # 更新每台机器的状态
        if not machine_id:
    ...
    def seq_decode(self, seq = None):   # 更新各个machine类当前的状态
        if not seq:
            ...
            self.machine_reset()
        # 双解码形式
        ms_seq = seq[0]   # 该层码是给工序附上机器的编号
        os_seq = seq[1]
        stack = []
        # 机器选择部分解码
        for job_id in range(self.num_jobs):
            for proc_id in range(self.num_process):
                idx = job_id * self.num_process + proc_id
                job_proc = self.get_process(job_id, proc_id)
                job_proc.machine_id = ms_seq[idx]
        # 工序选择部分解码
