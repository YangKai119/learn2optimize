...
def get_proc_time(j, df_data, model=None):
    ...
    proc_time = (tot_len + 75 * tot_num) / 4500
    return proc_time
...
def set_model(df_data):
    ...
    max_proc_time = (tot_len + 75 * tot_num) / 4500  # 边界处理
    model.x = pyo.Var(df_data.index, range(line_num), bounds=(0, df_data['F.No'].max()),
                      within=pyo.NonNegativeIntegers)  # 选择整版的数目
    model.u = pyo.Var(df_data.index, range(line_num), range(max_rest_stock), domain=pyo.Binary)  # 选择散版的数目
    model.y = pyo.Var(range(2), bounds=(0, max_proc_time), within=pyo.NonNegativeReals)  # 连续变量，因为要乘以长度，物理意义为平均处理时长
    # 目标函数
    ...
    for i in df_data.index:
        ...
        model.counts.add(expr=sum(model.u[i, j, idx] for idx in range(max_rest_stock) for j in range(line_num)) ==
                              df_data['R.No'].iloc[i])  # 散版的数量固定
        for idx in range(max_rest_stock):
            model.counts.add(expr=sum(model.u[i, j, idx] for j in range(line_num)) <= 1)  # 散板只能选择一个渠道进行投放
    # model.pprint()
    return model
